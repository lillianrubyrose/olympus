...
pub fn run(
	...
	match language {
		CompileLanguage::Rust => {
			gen_rust(&parser, output, overwrite, rs_crate, rs_crate_name)?;
		}
	}
...
fn gen_rust(
	...
	if gen_crate {
		return if let Some(crate_name) = gen_crate_name {
			gen_rust_crate(parser, &output, crate_name, overwrite)
		} else {
...
fn gen_rust_crate(parser: &Parser, output_dir: &Path, crate_name: &str, overwrite: bool) -> eyre::Result<()> {
...
"
	...
	std::fs::write(src_lib_path, format!("// {GENERATED_COMMENT}\npub mod models;"))?;
